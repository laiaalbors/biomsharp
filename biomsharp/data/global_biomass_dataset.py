...
@DATASET_REGISTRY.register()
class GlobalBiomassNaiveDataset(data.Dataset):
    def __init__(self, opt):
        super(GlobalBiomassNaiveDataset, self).__init__()
        self.opt = opt
        csv_path = opt['csv_path']
        first_row = opt['first_row'] if 'first_row' in opt else 0
        last_row = opt['last_row'] if 'last_row' in opt else None
    ...
    def __getitem__(self, idx):
        ...
        # pred_agb_25m = agb_100m.rio.reproject_match(agb_25m, resampling=Resampling.cubic).values
        ...
        # Baseline
        # upscaled_lq = np.nan_to_num(pred_agb_25m, copy=False, nan=-1.0).transpose(1, 2, 0)
