...
class SGNetModel(SRModel):
    ...
    def process(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output, self.out_grad = self.net_g_ema((self.gd, self.lq))
        else:
            self.net_g.eval()
            with torch.no_grad():
    ...
    def tile_process(self):
        ...
        self.out_grad = self.lq.new_zeros(output_shape)
            ...
            for x in range(tiles_x):
                ...
                try:
                    if hasattr(self, 'net_g_ema'):
                        self.net_g_ema.eval()
                        with torch.no_grad():
                            output_tile, out_grad_tile = self.net_g_ema((optical_tile, input_tile))
                    else:
                        self.net_g.eval()
                        with torch.no_grad():
                            output_tile, out_grad_tile = self.net_g((optical_tile, input_tile))
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.cri_pix:
            l_pix = self.cri_pix(self.output, self.out_grad, self.gt)
            l_total += l_pix
    ...
    def france_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # tentative for out of GPU memory
            del self.lq, self.gd, self.output, visuals
            if idx % 10 == 0:
                torch.cuda.empty_cache()
