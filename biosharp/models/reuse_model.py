...
class ReUseModel(SRModel):
    ...
    def process(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output = self.net_g_ema(self.gd)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.gd)
            self.net_g.train()
    ...
    def tile_process(self):
        ...
        """
        batch, channel, height, width = self.gd.shape
        output_height = height
        output_width = width
        output_shape = (batch, 1, output_height, output_width)

        # start with black image
        self.output = self.gd.new_zeros(output_shape)
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                input_tile = self.gd[:, :, input_start_y_pad:input_end_y_pad, input_start_x_pad:input_end_x_pad]
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            img_name = osp.splitext(osp.basename(val_data['gd_path'][0]))[0]
            ...
            sr_img = tensor2img([visuals['result']], min_max=(0, 563), out_type=np.uint16) # , min_max=(0, 563)
            metric_data['img'] = sr_img
            if 'gt' in visuals:
                gt_img = tensor2img([visuals['gt']], min_max=(0, 563), out_type=np.uint16) # , min_max=(0, 563)
